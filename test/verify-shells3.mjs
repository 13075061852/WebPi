...
for (const dev of ["desktop", "tablet", "mobile"]) {
  ...
  const s = await ev('(() => {' +
    ...
    'const sb = document.querySelector("#pvBody .dev-statusbar");' +
    'const sbStyle = sb ? getComputedStyle(sb) : null;' +
    'const cam = document.querySelector("#pvBody .dsb-cam");' +
    'return JSON.stringify({' +
    ' dev: "' + dev + '",' +
    ' shellBg: ss.backgroundImage.slice(0, 44) || ss.backgroundColor,' +
    ' statusbar: sb ? sbStyle.display + " h=" + sbStyle.height + " time=" + (sb.querySelector(".dsb-time") || {}).textContent : "none",' +
    ' cam: cam ? "9px circle" : "none",' +
    ' island: ss.height' +
    '});' +
  '})()');
  console.log(s);
