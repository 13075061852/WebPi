...
const row=[...document.querySelectorAll('#wsTree .dir')].find(r=>r.nextElementSibling?.childElementCount);
if(!row) throw Error('No populated folder');
const kids=row.nextElementSibling;
if(row.classList.contains('open')) {row.click();await wait(300);}
row.click();await wait(50);
const animating=kids.getAnimations().length>0;
row.click();await wait(30);row.click();await wait(300);
return {animating,connected:row.isConnected,open:row.classList.contains('open'),visible:!kids.hidden,height:kids.getBoundingClientRect().height};
})()`);
if(!result.animating||!result.connected||!result.open||!result.visible||result.height<=0)throw Error(JSON.stringify(result));
console.log('TREE ANIMATION PASS',JSON.stringify(result));
