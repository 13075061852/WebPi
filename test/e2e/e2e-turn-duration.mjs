...
try {
 await sleep(1200);
 const labels=await evalJS(`(async()=>{
 const start=1700000000000;
 const messages=[];
 for(let i=0;i<3;i++){const t=start+i*300000;messages.push({role:'user',timestamp:i===2?null:t,content:[{type:'text',text:'question'}]},{role:'assistant',timestamp:i===2?null:t+75000,content:[{type:'thinking',thinking:'analysis'},{type:'text',text:'answer'}]});}
 await window.__haloRestoreView({state:{ready:true,isStreaming:false},messages,turnTimings:{[String(start)]:{start,end:start+125000}}});
 return [...document.querySelectorAll('.process-label')].map(n=>n.textContent);
 })()`);
 if(JSON.stringify(labels)!==JSON.stringify(['用时 2分钟 5秒','约用时 1分钟 15秒','执行过程']))throw Error(JSON.stringify(labels));
 console.log('PASS stored duration, historical estimate and missing timestamps');
