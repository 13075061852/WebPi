...
for (let i = 0; i < 30; i++) {
  await sleep(700);
  rows = await ev('document.querySelectorAll("#wsTree .trow").length');
  if (rows > 0) break;
}
console.log("cold boot tree:", await ev('JSON.stringify({ wsPath: document.getElementById("wsPath").textContent, rows })'));
process.exit(0);
