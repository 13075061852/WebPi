...
await sleep(1200);
// 左侧会话列表应包含"新会话"
const list = await ev('JSON.stringify({ items: [...document.querySelectorAll("#sessionList .session-item .s-name")].map(e => e.textContent.trim()).slice(0, 4) })');
console.log("session list after new:", list);
process.exit(0);
