...
const expr = '(() => {' +
  ...
  'const dv = document.querySelector(".pv-devices");' +
  'const a = Math.round(mc.getBoundingClientRect().top);' +
  'const b = Math.round(dv.getBoundingClientRect().top);' +
  'return { modelCardTop: a, devicesTop: b, aligned: Math.abs(a - b) <= 1, mcH: Math.round(mc.getBoundingClientRect().height), dvH: Math.round(dv.getBoundingClientRect().height) };' +
'})()';
