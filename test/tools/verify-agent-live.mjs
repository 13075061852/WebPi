...
await sleep(4000);
console.log("set default:", await ev('(async () => JSON.stringify(await window.halo.agentDefaultSet("halo-test-agent")))()'));
// prompt 返回的 promise 在回复完成后 resolve
await ev('window.halo.prompt("用一句话自我介绍")');
const txt = await ev('(() => { const el = document.getElementById("messages"); return el ? el.innerText.slice(-500) : ""; })()');
