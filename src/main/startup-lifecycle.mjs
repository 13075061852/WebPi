...
export function createStartupLifecycle({ now = () => Math.round(process.uptime() * 1000), onChange = () => {}, reveal = () => {} } = {}) {
  const steps = [
  ...
  let painted = false, wired = false, shown = false, disposed = false, revision = 0;
  const snapshot = () => ({
